...
)
{
    ...
    for (int i = 0; i < num_boxes; ++i) {
        ...
        if (final_score > max_person_score) {
            max_person_score = final_score;
        }
    ...
    std::cout << "debug max obj_conf     : " << max_obj << std::endl;
    std::cout << "debug max person_score : " << max_person_score << std::endl;
...
int main(int argc, char** argv)
{
    ...
    std::string output_path = "outputs/cpp_detect_result.jpg";
    ...
    try {
        ...
        std::vector<Detection> results = postprocess_yolov5(
            output_data,
            num_boxes,
            element_per_box,
            orig_w,
        ...
        std::cout << "person_count: " << results.size() << std::endl;

        cv::Mat draw = image.clone();

        for (const auto& det : results) {
            cv::rectangle(draw, det.box, cv::Scalar(0, 255, 0), 2);
        ...
        cv::putText(
            draw,
            "C++ RKNN YOLOv5 Person Detection",
            cv::Point(20, 40),
            cv::FONT_HERSHEY_SIMPLEX,
            1.0,
            cv::Scalar(0, 255, 0),
        ...
        cv::putText(
            draw,
            "person_count: " + std::to_string(results.size()),
            cv::Point(20, 80),
            cv::FONT_HERSHEY_SIMPLEX,
            1.0,
            cv::Scalar(0, 255, 0),
        ...
        std::cout << "C++ image detection demo OK" << std::endl;
